...
These include number of replicas, storage and network resources, like
exposed ports to the outside. You can change the attributes while
runtime, without restarting a service. \cite{dock-swarm}

\paragraph{Task}\label{task}

A task is a the running container itself which is assigned to the
service. It is managed by the \emph{swarm manager}. \cite{dock-swarm}
