...
that expose a RESTful API and two supporting services in charge of
storing data. Figure \ref{img.overview_main_services} shows an overview
of these services.
...
route returns a JSON object containing all ViNNSL neural network
objects.
...
(nn-worker-service)}\label{worker-service-nn-worker-service}

The \texttt{nn-worker-service} implements a queue for neural network
training and evaluation and

\subsection{Storage Service
(nn-storage-service)}\label{storage-service-nn-storage-service}

\subsection{Frontend UI (vinnsl-ui)}\label{frontend-ui-vinnsl-ui}
